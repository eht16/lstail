...
########################################################################
class ElasticSearch7QueryBuilder(BaseQueryBuilder):

    # ----------------------------------------------------------------------
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._kibana_search_references = None
    ...
    def _fetch_index(self):
        ...
        # find index pattern in references
        for reference in self._kibana_search_references:
            if reference['type'] == 'index-pattern' and \
                    reference['name'] == 'kibanaSavedObjectMeta.searchSourceJSON.index':
                index_pattern_id = reference['id']
                break
